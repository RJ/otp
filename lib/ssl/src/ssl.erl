...
handle_options(Opts0, _Role) ->
    ...
    SslOptions = [versions, verify, verify_fun,
		  ...
		  cb_info, renegotiate_at, secure_renegotiate, hibernate_after, elr_dist, next_protocols_advertised,
...
validate_option(client_preferred_next_protocols, {FallbackProtocol, Order, PreferredProtocols} = Value)
      when is_list(PreferredProtocols), is_binary(FallbackProtocol), byte_size(FallbackProtocol) > 0 ->
...
validate_binary_list(Opt, List) ->
    lists:foreach(
        fun(Bin) when is_binary(Bin), byte_size(Bin) > 0 ->
            ok;
           (Bin) ->
